...
function EditUser(props) {
    ...
    useEffect(()=>{
        axios.get('/users/edit-user/' + props.match.params.id)
        .then(res=>{
            setUser({
                name: res.data.name,
    ...
    const handleSubmit = (event) => {
        ...
        // await promisedSetState(validate())
        // if(valid){
        //     console.log(user)
        // }else{
        ...
        valid = validate()
        if(valid){
            axios.put('/users/update-user/' + props.match.params.id, user)
            .then(
    ...
    const validate = () => {
        ...
        if (mobileError || nameError || emailError || genderError || technologyError) {
            setError({
                nameError: nameError,
    ...
    return (
        <div>
            ...
            <form>
                    ...
                    <div>
                    <button onClick={handleSubmit} type="submit" className="btn btn-success" >Update</button>
                    </div>
                </form>
        </div>
    )
